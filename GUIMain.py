...
class GUIMain:
    ...
    def setup_main_area(self):
        ...
        trial_button = tk.Button(main_area, text='trial', command=tomate)
        trial_button.grid(row=0, column=0)
    ...
    def setup_warnings_screen(self):
        ...
        # title_label = tk.Label(warnings_screen, text='Details', font=("bold", 14))
        # title_label.grid(row=0, column=0, sticky="w")

        scrollbar = tk.Scrollbar(warnings_screen)
        scrollbar.pack(side="right", fill="y")

        warning_list = tk.Listbox(warnings_screen, yscrollcommand=scrollbar.set)

        for line in range(100):
            warning_list.insert(tk.END, "This is line number " + str(line))
